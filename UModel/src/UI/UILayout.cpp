...
void UIGroup::ComputeLayout()
{
	...
	DBG_LAYOUT("this: Layout(%g %g %g %g), Rect(%g %g %g %g)",
        ...
        for (UIElement* child = FirstChild; child; child = child->NextChild)
        {
            ...
            MarginsSizeX += ((child == FirstChild) ? 0 : child->LeftMargin) + ((child->NextChild)
                                                                                   ? child->RightMargin
                                                                                   : 0);
        ...
        DBG_LAYOUT(">>> do layout: max_w(%d) max_h(%d) frac_w(%g) frac_h(%g)", MaxWidth, MaxHeight, TotalFracWidth,
                   TotalFracHeight);
