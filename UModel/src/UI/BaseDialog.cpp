...
HWND UICreateContext::MakeWindow(UIElement* control, const char* className, const char* text, DWORD style,
                                 DWORD exStyle, const UIRect* customRect)
{
    ...
    HWND wnd = CreateWindowEx(exStyle, className, text, style | WS_CHILDWINDOW, x, y, w, h,
                              parentWnd, (HMENU)(size_t)control->Id, hInstance,
                              NULL); // convert int -> size_t -> HANDLE to avoid warnings on 64-bit platform
...
HWND UICreateContext::MakeWindow(UIElement* control, const wchar_t* className, const wchar_t* text, DWORD style,
                                 DWORD exStyle, const UIRect* customRect)
{
    const UIRect* rect = customRect ? customRect : &control->Rect;
...
/*static*/
LONG_PTR CALLBACK UIElement::StaticSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
void UIBitmap::Create(UICreateContext& ctx)
{
    ...
    if (Wnd && hImage)
        SendMessage(Wnd, STM_SETIMAGE, IsIcon ? IMAGE_ICON : IMAGE_BITMAP, (LPARAM)hImage);
}
...
void UIProgressBar::SetValue(float value)
{
    ...
    Value = value;
    if (Wnd)
        SendMessage(Wnd, PBM_SETPOS, (int)(Value * 16384), 0);
}
...
UICombobox& UICombobox::AddItem(const char* item, int value)
{
    ...
    if (Wnd)
        SendMessage(Wnd, CB_ADDSTRING, 0, (LPARAM)item);
    return *this;
}
...
UIListbox& UIListbox::AddItem(const char* item)
{
    ...
    if (Wnd)
        SendMessage(Wnd, LB_ADDSTRING, 0, (LPARAM)item);
    return *this;
}
...
/*static*/
int UITreeView::GetHash(const char* text)
{
    int hash = 0;
    while (char c = *text++)
    {
...
static void LoadFolderIcons()
{
    ...
    GTreeFolderImages = ImageList_Create(GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON),
...
void UICheckboxGroup::UpdateLayout()
{
    ...
    MoveWindow(CheckboxWnd, Rect.X + checkboxOffset, Rect.Y - 2,
               min(checkboxWidth + DEFAULT_CHECKBOX_HEIGHT, Rect.Width - checkboxOffset), DEFAULT_CHECKBOX_HEIGHT,
               FALSE);
}
...
static DLGTEMPLATE* MakeDialogTemplate(int width, int height, const wchar_t* title, const wchar_t* fontName,
                                       int fontSize, bool resizeable)
{
