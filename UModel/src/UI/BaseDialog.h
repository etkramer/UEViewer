...
struct UICreateContext
{
    ...
    HWND MakeWindow(UIElement* control, const char* className, const char* text, DWORD style, DWORD exStyle,
                    const UIRect* customRect = NULL);
    // Unicode version of Window()
    HWND MakeWindow(UIElement* control, const wchar_t* className, const wchar_t* text, DWORD style, DWORD exStyle,
                    const UIRect* customRect = NULL);
...
class UIElement
{
    ...
    protected:
        ...
        void EnableSubclass();
        static LONG_PTR CALLBACK StaticSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
class UIMulticolumnListbox : public UIElement
{
    DECLARE_UI_CLASS(UIMulticolumnListbox, UIElement);
        ...
        DECLARE_CALLBACK(OnGetItemText, const char*&, int, int);
        // true virtual mode (#3): (char*& OutText, int ItemIndex, int SubItemIndex)
    ...
    public:
        ...
        UIMulticolumnListbox& AllowMultiselect()
        {
            Multiselect = true;
            return *this;
        }

        UIMulticolumnListbox& SetVirtualMode()
        {
            IsVirtualMode = true;
            return *this;
        }
...
class UITreeView : public UIElement
{
    ...
    public:
        ...
        UITreeView& UseFolderIcons()
        {
            bUseFolderIcons = true;
            return *this;
        }

        UITreeView& UseCheckboxes()
        {
            bUseCheckboxes = true;
            return *this;
        }

        UITreeView& SetItemHeight(int value)
        {
            ItemHeight = value;
            return *this;
        }

        UITreeView& HasRootNode(bool value)
        {
            bHasRootNode = value;
            return *this;
        }
...
class UIMenuItem
{
    typedef UIMenuItem ThisClass;
        ...
        DECLARE_CALLBACK(CheckboxCallback, bool);
        // this callback is executed for checkboxes, in addition to main 'Callback'
        DECLARE_CALLBACK(RadioCallback, int);
        // this callback is executed for RadioGroup when one of its children clicked
    public:
        enum EType
        {
            MI_Text,
        ...
        UIMenuItem& Expose(UIMenuItem*& var)
        {
            var = this;
            return *this;
        }
