...
int UE4UnversionedPackage(int verMin, int verMax)
{
    ...
    #endif
    appErrorNoLog(
        "Unversioned UE4 packages are not supported. Please restart UModel and select UE4 version in range %d-%d using UI or command line.",
        verMin, verMax);
    return -1;
}
...
int main(int argc, const char** argv)
{
	...
	if (argc < 2)
	{
        ...
        {
            CommandLineError(
                "too many arguments, please check your command line.\nYou specified: package=%s, object=%s, class=%s",
	...
	if (!GameFiles.Num())
	{
            ...
            guard(List)
                ...
                for (int packageIndex = 0; packageIndex < Packages.Num(); packageIndex++)
                {
                    ...
                    for (int i = 0; i < Package->Summary.ExportCount; i++)
                    {
                        ...
                        appPrintf("%4d %8X %8X %s %s\n", i, Exp.SerialOffset, Exp.SerialSize,
                                  Package->GetClassNameFor(Exp), *Exp.ObjectName);
                    }
                }
            unguard;
            return 0;
        ...
        bNoSupportedObjects = !UObject::GObjObjects.Num() && bShouldLoadObjects;
        #if HAS_UI
        ...
        if (!GApplication.FindObjectAndCreateVisualizer(1, GApplication.GuiShown, true))
        //!! don't need to pass GuiShown there
        {
    ...
    unguardf("umodel_build=%s", STR(GIT_REVISION));
    // using string constant to allow non-git builds (with GIT_REVISION 'unknown')
