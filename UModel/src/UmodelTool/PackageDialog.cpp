    ...
    private:
        ...
        void GetItemTextHandler(UIMulticolumnListbox* Sender, const char*& OutText, int ItemIndex, int SubItemIndex)
        {
            guard(UIPackageList::GetItemTextHandler)
                ...
                static FStaticString<MAX_PACKAGE_PATH> Buffer;
                // returning this value outside by pointer, so it is 'static'
                ...
                else if (file->IsPackageScanned)
                {
                    ...
                    {
                        case COLUMN_NumSkel:
                            value = file->NumSkeletalMeshes;
                            break;
                        case COLUMN_NumStat:
                            value = file->NumStaticMeshes;
                            break;
                        case COLUMN_NumAnim:
                            value = file->NumAnimations;
                            break;
                        case COLUMN_NumTex:
                            value = file->NumTextures;
                            break;
                    }
                    if (value != 0)
                    {
...
void UIPackageDialog::InitUI()
{
    guard(UIPackageDialog::InitUI)
        ...
        guard(FillFolderTree)
            ...
            for (const FString* Folder : Folders)
            {
                ...
                if (!DirectorySelected)
                {
                    ...
                    bool isUE4EnginePath = (strnicmp(*Path, "Engine", 6) == 0) || (strnicmp(*Path, "/Engine", 7) == 0)
        ...
        {
            PackageTree->Expand(*SelectedDir);
            //!! note: will not work at the moment because "Expand" works only after creation of UITreeView
        }
...
/*static*/
void UIPackageDialog::SortPackages(PackageList& List, int Column, bool Reverse)
{
...
void UIPackageDialog::OnColumnClick(UIMulticolumnListbox* sender, int column)
{
    ...
    {
        SortedColumn = column;
        ReverseSort = (column >= 1);
        // default sort mode for first (name) column is 'normal', for other columns - 'reverse'
    }
    SortPackages();
}
