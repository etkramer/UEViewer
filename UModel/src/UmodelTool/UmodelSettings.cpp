...
void CUmodelSettings::Load()
{
    guard(CUmodelSettings::Load)
        ...
        FArchive* Ar = new FFileReader(*ConfigFile, EFileArchiveOptions::NoOpenError);
        // can't use TextFile because of FFileReader::IsEof will not work fine with it
        if (!Ar->IsOpen())
        {
            delete Ar;
            return;
