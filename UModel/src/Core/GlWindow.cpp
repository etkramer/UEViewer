...
static void OnMouseButton(int type, int button)
{
    ...
    if (!prevButtons && Viewport::MouseButtons)
    {
        ...
        // switch to relative mode - mouse cursor will be hidden and remains in single place
    }
...
static void Init(const char* caption)
{
    guard(SDL.Init)
        ...
        sdlWindow = SDL_CreateWindow(caption,
                                     SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, Viewport::Size.X,
                                     Viewport::Size.Y,
            ...
            while (SDL_PollEvent(&evt))
            {
				...
				case SDL_WINDOWEVENT_RESIZED:
                    ...
                    case SDL_QUIT:
                        ...
                        #if _WIN32
                    case SDL_SYSWMEVENT:
                        WndProc(evt.syswm.msg->msg.win.msg, evt.syswm.msg->msg.win.wParam,
                                evt.syswm.msg->msg.win.lParam);
                        break;
                        #endif
                }
            }
