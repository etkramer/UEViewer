        ...
        void RenderChar(char Ch, unsigned Color, int TextX, int TextY)
        {
            ...
            for (int IsShadow = ShadowCount; IsShadow >= 0; IsShadow--)
            {
                ...
                *pColorData++ = ActiveColor;
                ...
                pIndexData[0] = CurrentVertex;
                pIndexData[1] = CurrentVertex + 1;
                pIndexData[2] = CurrentVertex + 2;
                pIndexData[3] = CurrentVertex;
                pIndexData[4] = CurrentVertex + 2;
                pIndexData[5] = CurrentVertex + 3;
                pIndexData += 6;
...
{
    if (GUseGLSL)
        glUseProgram(0); //?? default shader will not allow alpha on text
    // setup GL
...
static void DrawTextPos(int x, int y, const char* text, unsigned color, bool bHyperlink, bool bHighlightLink,
                        ETextAnchor anchor)
{
...
static bool DrawTextAtAnchor(ETextAnchor anchor, unsigned color, bool bHyperlink, bool* pHover, const char* fmt,
                             va_list argptr)
{
