...
void ExportPsk(const CSkeletalMesh* Mesh)
{
    ...
    {
        FArchive* PropAr = CreateExportArchive(OriginalMesh, EFileArchiveOptions::TextFile, "%s.props.txt",
                                               OriginalMesh->Name);
        if (PropAr)
        {
...
static void DoExportPsa(const CAnimSet* Anim, const UObject* OriginalAnim)
{
    guard(DoExportPsa)
                ...
                guard(Sequence)
                    ...
                    for (int t = 0; t < S.NumFrames; t++)
                    {
                        for (int b = 0; b < numBones; b++)
                        {
                            ...
                            BP.Set(0, 0, 0);
                            // GetBonePosition() will not alter BP and BO when animation tracks are not exists
                            BO.Set(0, 0, 0, 1);
        ...
        {
            FArchive* PropAr = CreateExportArchive(OriginalAnim, EFileArchiveOptions::TextFile, "%s.props.txt",
                                                   OriginalAnim->Name);
            if (PropAr)
            {
