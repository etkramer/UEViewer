...
enum EFbxExportCompatibility : uint8_t
{
    FBX_2010,
    FBX_2011,
    FBX_2012,
...
{
    public:
        static void SetJointPostConversionMatrix(const FbxAMatrix& ConversionMatrix)
        {
...
class FFbxExporter
{
    ...
    private:
        ...
        */
        FbxNode* ExportStaticMeshToFbx(const CStaticMesh* StaticMesh, int32 ExportLOD, const char* MeshName,
                                       FbxNode* FbxActor, int32 LightmapUVChannel = -1);
        FbxSurfaceMaterial* CreateDefaultMaterial(int Index);
        FbxSurfaceMaterial* ExportMaterial(UUnrealMaterial* Material);
        bool FillFbxTextureProperty(const char* PropertyName, const UUnrealMaterial* Texture,
