...
void ExportMd5Mesh(const CSkeletalMesh* Mesh)
{
    guard(ExportMd5Mesh)
        ...
        FArchive* Ar = CreateExportArchive(OriginalMesh, EFileArchiveOptions::TextFile, "%s.md5mesh",
                                           OriginalMesh->Name);
        if (!Ar) return;
...
void ExportMd5Anim(const CAnimSet* Anim)
{
    guard(ExportMd5Anim)
        ...
        for (int AnimIndex = 0; AnimIndex < Anim->Sequences.Num(); AnimIndex++)
        {
            ...
            FArchive* Ar = CreateExportArchive(OriginalAnim, EFileArchiveOptions::TextFile, "%s/%s.md5anim",
                                               OriginalAnim->Name, *S.Name);
            if (!Ar)
            {
                if (AnimIndex == 0)
                    // if file overwrite is disabled and file already exists, don't save animations at all
                    return;
