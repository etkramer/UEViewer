...
void WriteTGA(FArchive& Ar, int width, int height, byte* pic)
{
    guard(WriteTGA)
        ...
        byte* packed = (byte*)appMalloc(width * height * colorBytes + 4);
        // +4 for being able to put uint32 even when 3 bytes needed
    ...
    void operator()()
    {
        ...
        for (int Slice = 0; Slice < SliceCount; Slice++)
        {
            if (Slice >= 1)
            {
                ...
                appSprintf(ARRAY_ARG(FullPath), "%s/%s/Side_%d.%s", *ExportPath, TexData.GetObjectName(), Slice,
                           *ExportExt);
