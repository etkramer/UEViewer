    ...
    , HighlightMeshIndex(-1)
{
    guard(CSkelMeshViewer::CSkelMeshViewer)
        ...
        TimeSinceCreate = -2;
        // ignore first 2 frames: 1st frame will be called after mesh changing, 2nd frame could load textures etc
...
void CSkelMeshViewer::Draw2D()
{
    ...
    if (bIsUE4Mesh)
    {
        if (Skeleton)
        {
            bool bClicked = DrawTextH(ETextAnchor::TopLeft, NULL, S_GREEN "Skeleton: " S_WHITE S_HYPERLINK("%s"),
                                      Skeleton->Name);
            if (bClicked)
                JumpTo(Skeleton);
        }
    ...
    for (int i = 0; i < TaggedMeshes.Num(); i++)
    {
        ...
        bool bHighlight;
        bool bClicked = DrawTextH(ETextAnchor::TopLeft, &bHighlight, "%s%d: " S_HYPERLINK("%s"),
                                  (TaggedMesh == MeshInst->pMesh) ? S_RED : S_WHITE, i, TaggedMesh->OriginalMesh->Name);
        if (bHighlight)
        {
    ...
    if (Anim)
    {
        ...
        if (DrawTextH(ETextAnchor::BottomLeft, NULL, S_GREEN "AnimSet: " S_WHITE S_HYPERLINK("%s"),
                      Anim->OriginalAnim->Name))
        {
        ...
        if (Seq)
        {
            if (Seq->OriginalSequence)
            {
                ...
                bool bClicked = DrawTextH(ETextAnchor::BottomLeft, NULL, S_GREEN "Anim:" S_WHITE " %d/%d "
                                          ...
                                          AnimIndex + 1, MeshInst->GetAnimCount(), *Seq->Name, Seq->Rate,
                                          Seq->NumFrames,
    ...
    if (Mesh->Morphs.Num())
    {
        ...
        {
            DrawTextBottomLeft(S_GREEN "Morph: " S_WHITE " %d/%d (%s)", MorphIndex + 1, MorphCount,
                               *Mesh->Morphs[MorphIndex]->Name);
        }
        else
        {
        ...
        while (true)
        {
            ...
            if (found && NewAnim)
            {
                ...
                if (NewAnim->Sequences.Num())
                {
                    ...
                    SetAnim(NewAnim);
                    AnimIndex = -1;
                    appPrintf("Bound %s'%s' to %s'%s'\n", Object->GetClassName(), Object->Name, Obj->GetClassName(),
                              Obj->Name);
                    break;
                }
                else
...
void CSkelMeshViewer::FindUE4Animations()
{
    ...
    guard(CSkelMeshViewer::FindUE4Animations)
        ...
        appEnumGameFiles<TArray<const CGameFileInfo*>>(
            // won't compile with lambda without explicitly providing template argument
