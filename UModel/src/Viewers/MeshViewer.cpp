...
void CMeshViewer::Draw3D(float TimeDelta)
{
    guard(CMeshViewer::Draw3D)
        ...
        //?? bWireframe is inside Inst, but used here only ?
        DrawMesh(Inst);
...
void CMeshViewer::PrintMaterialInfo(int Index, UUnrealMaterial* Material, int NumFaces)
{
    ...
    if (Material)
    {
        bool bClicked = DrawTextH(ETextAnchor::TopLeft, &bHighlight, color, "  %d: " S_HYPERLINK("%s (%s)") ", %d tris",
                                  Index, Material->Name, Material->GetClassName(), NumFaces);
        if (bClicked)
            JumpTo(Material);
    }
    else
