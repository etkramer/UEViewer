                         ...
                         int* Sizes, int* Depth, int& numIndices, int maxIndices)
{
    ...
    if (currIndex != Index) appNotify("Strange skeleton, check childs of bone %d (%s)", currIndex,
                                      *Bones[currIndex].Name);
...
void CSkelMeshInstance::SetMesh(CSkeletalMesh* Mesh)
{
    guard(CSkelMeshInstance::SetMesh)
        ...
        int DataSize = sizeof(CMeshBoneData) * NumBones + sizeof(CSkinVert) * NumVerts + sizeof(CSkelMeshVertex) *
            NumMorphVerts;
	...
	memset(BoneDebugInfos, 0, NumBones * sizeof(CBoneDebugInfo));
        ...
        for (Stage = 0, Chn = Channels; Stage <= MaxAnimChannel; Stage++, Chn++)
        {
			...
			BoneDebug.AnimRotation = Bone.Orientation;
                ...
                if (AnimSeq1 && AnimBoneIndex != INDEX_NONE && AnimSeq1->Tracks.IsValidIndex(AnimBoneIndex) && AnimSeq1
                    ->Tracks[AnimBoneIndex]->HasKeys())
                {
                    ...
                    // Current state:
                    EBoneRetargetingMode RetargetingMode = Animation->GetBoneTranslationMode(
                        AnimBoneIndex, (EAnimRetargetingMode)RetargetingModeOverride);
                    #if SHOW_ANIM
    ...
    for (Stage = 0, Chn = Channels; Stage <= MaxAnimChannel; Stage++, Chn++)
    {
        ...
        if (!Chn->TweenTime && Chn->Anim1)
        {
            ...
            if (Seq2)
            {
                ...
                EffectiveRate = Lerp(fabs(Seq1->Rate) / Seq1->NumFrames, fabs(Seq2->Rate) / Seq2->NumFrames,
                                     Chn->SecondaryBlend)
                    * Seq1->NumFrames;
            }
...
void CSkelMeshInstance::GetAnimParams(int Channel, const char*& AnimName, float& Frame, float& NumFrames,
                                      float& Rate) const
{
...
void CSkelMeshInstance::DrawMesh(unsigned flags)
{
    guard(CSkelMeshInstance::DrawMesh)
        ...
        /*const*/
        CSkelMeshLod& Mesh = pMesh->Lods[LodIndex]; //?? not 'const' because of BuildTangents(); change this?
	...
	DrawTextLeft(S_RED"Show section: %d (%d faces)", ShowSection, Sections[ShowSection].NumFaces);
		...
		int MaterialIndex = i;
            ...
            if (Mesh.Indices.Is32Bit())
                glDrawElements(GL_TRIANGLES, Sec.NumFaces * 3, GL_UNSIGNED_INT,
                               &Mesh.Indices.Indices32[Sec.FirstIndex]);
            else
                glDrawElements(GL_TRIANGLES, Sec.NumFaces * 3, GL_UNSIGNED_SHORT,
                               &Mesh.Indices.Indices16[Sec.FirstIndex]);
            // disable tangents
            if (aNormal >= 0)
