                   ...
                   bool bgra)
{
    guard(UncompressPNG)
        ...
        png_structp png_ptr = png_create_read_struct_2(
            PNG_LIBPNG_VER_STRING, &ctx, user_error_fn, user_warning_fn, NULL, user_malloc, user_free);
        assert(png_ptr);
        ...
        png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
        png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, &ctx, user_error_fn, user_warning_fn, NULL,
...
void CompressPNG(const unsigned char* pic, int Width, int Height, TArray<byte>& CompressedData)
{
    guard(CompressPNG)
        ...
        png_set_IHDR(png_ptr, info_ptr, Width, Height, RawBitDepth,
                     (PixelChannels == 4) ? PNG_COLOR_TYPE_RGBA : PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
