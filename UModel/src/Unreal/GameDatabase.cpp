...
const GameInfo GListOfGames[] = {
    ...
    // Dummy tag for all UE4 versions
    {
        ...
        GAME_UE4(LATEST_SUPPORTED_UE4_VERSION+1)
        // some invalid version number, but not zero - to show this game in "-help" output
    },
...
void FArchive::DetectGame()
{
	...
	if (ArVer == 119 && ArLicenseeVer == 0x9127)
        ...
        #endif
        #if ARGONAUTS
        if (ArVer == 539 && (ArLicenseeVer == 43 || ArLicenseeVer == 47)) SET(GAME_Argonauts);
        // Rise of the Argonauts, Thor: God of Thunder
        #endif
        #if ALPHA_PR
        ...
        #endif
        #if FABLE
        if ((ArVer == 850 || ArVer == 860) && (ArLicenseeVer == 1017 || ArLicenseeVer == 26985))
        // 850 = Fable: The Journey, 860 = Fable Anniversary
            SET(GAME_Fable);
        #endif
        #if MURDERED
        ...
        #endif
        #if LOST_PLANET3
        if (ArVer == 860 && (ArLicenseeVer == 97 || ArLicenseeVer == 98))
        // 97 = Lost Planet 3, 98 = Yaiba: Ninja Gaiden Z
...
void FArchive::OverrideVersion()
{
    ...
    #endif
    #if TRANSFORMERS
    if (Game == GAME_Transformers && ArLicenseeVer >= 181) ArVer = OVERRIDE_TRANSFORMERS3;
