...
static void appDecompressOodle_DLL(byte* CompressedBuffer, int CompressedSize, byte* UncompressedBuffer,
                                   int UncompressedSize)
{
    guard(appDecompressOodle_DLL)
        ...
        if (!bOodleLoaded)
        {
            ...
            hOodleDll = LoadLibraryA(OodleDllName);
            ...
            if (!hOodleDll)
            {
                ...
                {
                    hOodleDll = LoadLibraryA(va("%s\\%s", Path, OodleDllName));
                    if (hOodleDll) break;
                }
            }
...
static int DetectCompressionMethod(byte* CompressedBuffer)
{
    ...
    {
        Flags = COMPRESS_LZ4;
        // in most cases UE4 games are using either oodle or lz4 - the first one is explicitly recognizable
    }
    #endif // USE_LZ4
    else
    {
...
int appDecompress(byte* CompressedBuffer, int CompressedSize, byte* UncompressedBuffer, int UncompressedSize, int Flags)
{
    ...
    guard(appDecompress)
        ...
        #if BLADENSOUL
        if (GForceGame == GAME_BladeNSoul && Flags == COMPRESS_LZO_ENC_BNS)
        // note: GForceGame is required (to not pass 'Game' here)
        {
    ...
    restart_decompress:
        ...
        {
            int newLen = LZ4_decompress_safe((const char*)CompressedBuffer, (char*)UncompressedBuffer, CompressedSize,
                                             UncompressedSize);
            if (newLen <= 0)
	...
	return 0;
            ...
            else
            {
                ...
                appNotify("appDecompress: unknown compression flags %X, detected %X, retrying ...", Flags,
                          FoundCompression);
                Flags = FoundCompression;
            }
            Flags = FoundCompression;
    ...
    unguardf("CompSize=%d UncompSize=%d Flags=0x%X Bytes=%02X%02X", CompressedSize, UncompressedSize, OldFlags,
                 CompressedBuffer[0], CompressedBuffer[1]);
}
