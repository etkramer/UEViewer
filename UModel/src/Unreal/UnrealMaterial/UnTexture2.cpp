...
void UTexture::Serialize(FArchive& Ar)
{
    guard(UTexture::Serialize)
        ...
        if (Ar.Game == GAME_Bioshock && Format == 12)
            // remap format; note: Bioshock used 3DC name, but real format is DXT5N
            Format = TEXF_DXT5N;
        #endif // BIOSHOCK
        #if SWRC
...
bool UTexture::GetTextureData(CTextureData& TexData) const
{
	...
	if (TexData.Mips.Num() && TexData.Platform == PLATFORM_XBOX360)
	{
        ...
        if (Package && PackageAr->Game == GAME_Bioshock)
        {
            ...
            int numBlocks = TexData.Mips[0].USize * TexData.Mips[0].VSize / (Info.BlockSizeX * Info.BlockSizeY);
            ...
            if (requiredDataSize > TexData.Mips[0].DataSize)
            {
                appNotify("Bioshock texture %s: data too small; %dx%d, requires %X bytes, got %X\n",
                          Name, TexData.Mips[0].USize, TexData.Mips[0].VSize, requiredDataSize,
                          TexData.Mips[0].DataSize);
            }
