...
void UTexture2D::Serialize4(FArchive& Ar)
{
    guard(UTexture2D::Serialize4)
        ...
        FStripDataFlags StripFlags(Ar);
        // note: these flags are used for pre-VER_UE4_TEXTURE_SOURCE_ART_REFACTOR versions
        ...
        else if (SourceArt.BulkData != NULL)
        {
            guard(UTexture4::LoadSourceData)
                ...
                appPrintf("... Loading SourceArt: %s, NumMips=%d, Slices=%d, PNGCompressed=%d\n", FormatName,
                          Source.NumMips, Source.NumSlices, Source.bPNGCompressed);
                ...
                for (const UObject* Obj : Expressions)
                {
                    ...
                    if (Obj->IsA("MaterialExpressionTextureSampleParameter"))
                    {
                        const UMaterialExpressionTextureSampleParameter2D* Expr = static_cast<const
                            UMaterialExpressionTextureSampleParameter2D*>(Obj);
                    ...
                    else if (Obj->IsA("MaterialExpressionTextureSample"))
                    {
                        ...
                        const UMaterialExpressionTextureSample* Expr = static_cast<const
                            UMaterialExpressionTextureSample*>(Obj);
                        if (Expr->Texture) ReferencedTextures.AddUnique(Expr->Texture);
                    }
                    else if (Obj->IsA("MaterialExpressionScalarParameter"))
                    {
                        const UMaterialExpressionScalarParameter* Expr = static_cast<const
                            UMaterialExpressionScalarParameter*>(Obj);
                    ...
                    {
                        const UMaterialExpressionVectorParameter* Expr = static_cast<const
                            UMaterialExpressionVectorParameter*>(Obj);
