...
/*static*/
void FTexture2DMipMap::Serialize3(FArchive& Ar, FTexture2DMipMap& Mip)
{
...
void UTexture3::Serialize(FArchive& Ar)
{
    ...
    guard(UTexture3::Serialize)
        ...
        #endif // APB
        #if MASSEFF
        if ((Ar.Game == GAME_MassEffect3) || (Ar.Game == GAME_MassEffectLE && Ar.ArLicenseeVer >= 205)) return;
        // ME3 or ME3LE
        #endif
        #if BIOSHOCK3
...
void UTexture2D::Serialize(FArchive& Ar)
{
    guard(UTexture2D::Serialize)
        ...
        #if BORDERLANDS
        if (Ar.Game == GAME_Borderlands && Ar.ArLicenseeVer >= 46) Ar.Seek(Ar.Tell() + 16);
        // Borderlands 1,2; some hash; version unknown!!
        #endif
        #if MKVSDC
...
bool UTexture2D::LoadBulkTexture(const TArray<FTexture2DMipMap>& MipsArray, int MipIndex, const char* tfcSuffix,
                                 bool verbose) const
{
    ...
    guard(UTexture2D::LoadBulkTexture)
        ...
        if (verbose)
        {
            appPrintf("Reading %s mip level %d (%dx%d) from %s\n", Name, MipIndex, Mip.SizeX, Mip.SizeY,
                      *bulkFile->GetRelativeName());
        }
        ...
        if (TexData.Mips.Num() == 0 && MipsArray->Num())
        {
            ...
            for (int mipLevel = 0; mipLevel < MipsArray->Num(); mipLevel++)
            {
                ...
                if (!Mip.Data.BulkData)
                {
                    ...
                    // equals to BULKDATA_PayloadAtEndOfFile for UE4
