                ...
                if (Ar.Game == GAME_Lineage2)
                {
                    guard(SerializeLineage2Material)
                        ...
                        if (Ar.ArVer >= 123 && Ar.ArLicenseeVer >= 30 && Ar.ArLicenseeVer < 37)
                        {
                            int i;
                            // some function
                            byte MaterialInfo, TextureTranform, MAX_SAMPLER_NUM, MAX_TEXMAT_NUM, MAX_PASS_NUM,
                                 TwoPassRenderState, AlphaRef;
                            if (Ar.ArLicenseeVer >= 33 && Ar.ArLicenseeVer < 36)
                                Ar << MaterialInfo;
                            Ar << TextureTranform << MAX_SAMPLER_NUM << MAX_TEXMAT_NUM << MAX_PASS_NUM <<
                                TwoPassRenderState << AlphaRef;
