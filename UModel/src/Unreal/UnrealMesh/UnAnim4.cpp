        ...
        for (int BoneIndex = 0; BoneIndex < ReferenceSkeleton.RefBoneInfo.Num(); BoneIndex++)
        {
            ...
            if (Seq->KeyEncodingFormat == AKF_PerTrackCompression)
            {
                ...
                guard(PerTrackCompression)
                    ...
                    int ScaleOffset = Seq->CompressedScaleOffsets.IsValid()
                                          ? Seq->CompressedScaleOffsets.OffsetData[TrackIndex]
                                          : -1;
                    #if 0
			...
			if (ScaleOffset >= 0) { Reader.Seek(ScaleOffset); DUMP_ARC_BYTES(Reader, BytesToDump, "Scale"); }
                    ...
                    guard(TransKeys)
                        ...
                        else
                        {
                            ...
                            ReadPerTrackVectorData(Reader, TrackIndex, "translation", A->KeyPos, A->KeyPosTime,
                                                   Seq->NumFrames);
                        }
                    unguard;
                    ...
                    guard(RotKeys)
                        ...
                        else
                        {
                            ...
                            ReadPerTrackQuatData(Reader, TrackIndex, "rotation", A->KeyQuat, A->KeyQuatTime,
                                                 Seq->NumFrames);
                        }
                    unguard;
                ...
                for (int k = 0; k < TransKeys; k++)
                {
                    switch (TranslationCompressionFormat)
                    {
                        ...
                        case ACF_Identity:
                            ...
                            break;
                        default:
                            appError("Unknown translation compression method: %d (%s)", TranslationCompressionFormat,
                                     EnumToName(TranslationCompressionFormat));
                    }
                }
...
void UAnimSequenceBase::Serialize(FArchive& Ar)
{
    guard(UAnimSequenceBase::Serialize)
        ...
        // useful only between VER_UE4_SKELETON_ADD_SMARTNAMES and FFrameworkObjectVersion::SmartNameRefactor
...
void UAnimSequence4::Serialize(FArchive& Ar)
{
    guard(UAnimSequence4::Serialize)
        ...
        if (FFrameworkObjectVersion::Get(Ar) < FFrameworkObjectVersion::MoveCompressedAnimDataToTheDDC)
        {
            ...
            #if SEAOFTHIEVES
            if ((Ar.Game == GAME_SeaOfThieves) && (CompressedByteStream.Num() == 1) && (Ar.GetStopper() - Ar.Tell() >
                0))
            {
...
void UAnimSequence4::SerializeCompressedData(FArchive& Ar)
{
    ...
    no_raw_data_size:
        ...
        // Fix layout of "byte swapped" data (workaround for UE4 bug)
        if (KeyEncodingFormat == AKF_PerTrackCompression && CompressedScaleOffsets.OffsetData.Num() && Ar.Game <
            GAME_UE4(23))
        {
            TArray<uint8> SwappedData;
            TransferPerTrackData(SwappedData, CompressedByteStream);
    ...
    guard(UAnimSequence4::TransferPerTrackData)
        ...
        for (int TrackIndex = 0; TrackIndex < NumTracks; TrackIndex++)
        {
			...
			{ \
                ...
                static const int NumComponentsPerMask[8] = {3, 1, 1, 2, 1, 2, 2, 3};
                // number of identity bits in value, 0 == all bits
                ...
                switch (KeyFormat)
                {
                    ...
                    case ACF_Fixed32NoW:
                    case ACF_Float32NoW:
                        COPY(sizeof(uint32) * NumKeys);
                    // always stored full data, ComponentMask used only for mins/ranges
                        break;
                    case ACF_Identity:
                        // nothing
                        break;
...
{
    return CompressedTrackToSkeletonMapTable.Num()
               ? CompressedTrackToSkeletonMapTable.Num()
               : TrackToSkeletonMapTable.Num();
}
...
{
    const TArray<FTrackToSkeletonMap>& TrackMap = CompressedTrackToSkeletonMapTable.Num()
