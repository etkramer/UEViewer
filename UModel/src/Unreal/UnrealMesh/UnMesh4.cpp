...
struct FStaticMeshVertexBuffer4
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FStaticMeshVertexBuffer4& S)
    {
        guard(FStaticMeshVertexBuffer4<<)
            ...
            DBG_MESH("UV stream: TC:%d IS:%d NV:%d FloatUV:%d HQ_Tangent:%d\n", S.NumTexCoords, S.Stride, S.NumVertices,
                     S.bUseFullPrecisionUVs, S.bUseHighPrecisionTangentBasis);
...
struct FApexClothPhysToRenderVertData // new structure name: FMeshToMeshVertData
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FApexClothPhysToRenderVertData& V)
    {
        ...
        Ar << V.SimulMeshVertIndices[0] << V.SimulMeshVertIndices[1] << V.SimulMeshVertIndices[2] << V.
            SimulMeshVertIndices[3];
...
struct FSkelMeshSection4
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FSkelMeshSection4& S)
    {
        guard(FSkelMeshSection4<<)
            ...
            if (FRecomputeTangentCustomVersion::Get(Ar) >=
                FRecomputeTangentCustomVersion::RecomputeTangentVertexColorMask)
            {
            ...
            if (SkelMeshVer >= FSkeletalMeshCustomVersion::CombineSectionWithChunk)
            {
                ...
                if (!StripFlags.IsEditorDataStripped())
                {
                    ...
                    GNumSkelInfluences = (Ar.ArVer >= VER_UE4_SUPPORT_8_BONE_INFLUENCES_SKELETAL_MESHES)
                                             ? MAX_TOTAL_INFLUENCES_UE4
                                             : NUM_INFLUENCES_UE4;
                    Ar << S.SoftVertices;
                }
                Ar << S.BoneMap;
                ...
                if (FOverlappingVerticesCustomVersion::Get(Ar) >=
                    FOverlappingVerticesCustomVersion::DetectOVerlappingVertices)
                {
    ...
    static void SerializeRenderItem(FArchive& Ar, FSkelMeshSection4& S)
    {
        guard(FSkelMeshSection4::SerializeRenderItem)
            ...
            if (Ar.Game < GAME_UE4(23) || !StripFlags.IsClassDataStripped(1))
            // DuplicatedVertices, introduced in UE4.23
            {
            ...
            if (!StripFlags.IsDataStrippedForServer() && !StripFlags.IsClassDataStripped(CDSF_MinLodData))
            {
                ...
                guard(BuildVertexData)
                    ...
                    for (int i = 0; i < Lod.NumVertices; i++)
                    {
                        ...
                        static_assert(sizeof(V.Normal) == sizeof(SV.Normal),
                                      "FGPUVert4Common.Normal should match FStaticMeshUVItem4.Normal");
                        memcpy(V.Normal, SV.Normal, sizeof(V.Normal));
                        static_assert(sizeof(V.UV[0]) == sizeof(SV.UV[0]),
                                      "FGPUVert4Common.UV should match FStaticMeshUVItem4.UV");
                        static_assert(sizeof(V.UV) <= sizeof(SV.UV), "SkeletalMesh has more UVs than StaticMesh");
                        // this is just for correct memcpy below
			...
			for (int i1 = 0; i1 < Lod.Sections.Num(); i1++)
			{
                ...
                else
                {
                    ...
                    if (Bulk.ElementCount > 0)
                    {
                        ...
                        FMemReader Reader(Bulk.BulkData, Bulk.ElementCount);
                        // ElementCount is the same as data size, for byte bulk data
            ...
            guard(ConvertLod)
                ...
                guard(ProcessSections)
                    ...
                    for (int Sec = 0; Sec < SrcLod.Sections.Num(); Sec++)
                    {
                        ...
                        Dst->Material = Materials.IsValidIndex(MaterialIndex)
...
struct FStaticMeshLODModel4
{
    ...
    static void SerializeBuffersLegacy(FArchive& Ar, FStaticMeshLODModel4& Lod, const FStripDataFlags& StripFlags)
    {
        ...
        after_color_stream:
            ...
            /// 25.09.2015 - 948c1698
            if (Ar.ArVer >= VER_UE4_SOUND_CONCURRENCY_PACKAGE && !StripFlags.IsClassDataStripped(
                CDSF_ReversedIndexBuffer))
            {
...
void UStaticMesh4::Serialize(FArchive& Ar)
{
    ...
    no_nav_collision:
            ...
            for (int i = 0; i < SourceModels.Num(); i++)
            {
                ...
                else
                {
                    ...
                    if (bIsValid)
                    {
                        ...
                        if (FEnterpriseObjectVersion::Get(Ar) >=
        ...
        if (bCooked)
        {
            ...
            Lods.Serialize2<FStaticMeshLODModel4::Serialize>(Ar);
            // original code: TArray<FStaticMeshLODResources> LODResources
        ...
        if (Ar.Game >= GAME_UE4(14) /* && StaticMaterials.Num() == 0 */)
        // it seems that StaticMaterials serialized as properties has no material links
        {
...
void UStaticMesh4::ConvertMesh()
{
    guard(UStaticMesh4::ConvertMesh)
        ...
        // convert bounds
        Mesh->BoundingSphere.R = Bounds.SphereRadius / 2;
        //?? UE3 meshes has radius 2 times larger than mesh itself; verify for UE4
        VectorSubtract(CVT(Bounds.Origin), CVT(Bounds.BoxExtent), CVT(Mesh->BoundingBox.Min));
        VectorAdd(CVT(Bounds.Origin), CVT(Bounds.BoxExtent), CVT(Mesh->BoundingBox.Max));
    ...
    guard(UStaticMesh4::ConvertSourceModels)
        ...
        for (int LODIndex = 0; LODIndex < SourceModels.Num(); LODIndex++)
        {
            guard(ConvertLod)
                ...
                Lod->HasTangents = !SrcModel.BuildSettings.bRecomputeTangents && (RawMesh.WedgeTangent.Num() > 0) && (
                        RawMesh.WedgeBinormal.Num() > 0)
                    && Lod->HasNormals;
...
/*static*/
void FMorphTargetDelta::Serialize4(FArchive& Ar, FMorphTargetDelta& V)
{
    Ar << V.PositionDelta;
...
/*static*/
void FMorphTargetLODModel::Serialize4(FArchive& Ar, FMorphTargetLODModel& Lod)
{
