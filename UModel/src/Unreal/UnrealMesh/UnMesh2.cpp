...
void USkeletalMesh::Serialize(FArchive& Ar)
{
	...
	for (int i1 = 0; i1 < RefSkeleton.Num(); i1++)
        ...
        #if LOCO
        if (Ar.Game == GAME_Loco) goto skip_remaining;
...
void USkeletalMesh::ConvertWedges(CSkelMeshLod& Lod, const TArray<FVector>& MeshPoints,
                                  const TArray<FMeshWedge>& MeshWedges, const TArray<FVertInfluence>& VertInfluences)
{
...
void UStaticMesh::Serialize(FArchive& Ar)
{
    guard(UStaticMesh::Serialize)
        ...
        #endif
        #if VANGUARD
        if (Ar.Game == GAME_Vanguard) GUseNewVanguardStaticMesh = false;
        // in game code InternalVersion is analyzed before serialization
        #endif
        Ar << Sections;
