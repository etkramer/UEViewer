...
void UAnimSequence::Serialize(FArchive& Ar)
{
    guard(UAnimSequence::Serialize)
        ...
        #endif
        #if MASSEFF
        if ((Ar.Game == GAME_MassEffect2 && Ar.ArLicenseeVer >= 110) || (Ar.Game == GAME_MassEffectLE && Ar.
            ArLicenseeVer == 168)) // ME2 or ME2LE
        {
        ...
        if (Ar.Game == GAME_MassEffect3 || Ar.Game == GAME_MassEffectLE) goto old_code;
        // Mass Effect 3 has no RawAnimationData
        #endif // MASSEFF
        #if MOH2010
        if (Ar.Game == GAME_MOH2010) goto old_code;
        #endif
        #if TERA
        if (Ar.Game == GAME_Tera && Ar.ArLicenseeVer >= 11) goto new_code;
        // we have overridden ArVer, so compare by ArLicenseeVer ...
        #endif
        #if TRANSFORMERS
        if (Ar.Game == GAME_Transformers && Ar.ArLicenseeVer >= 181)
        // Transformers: Fall of Cybertron, no version in code
        {
            int UseNewFormat;
            Ar << UseNewFormat;
            if (UseNewFormat)
...
static void ReadArgonautsTimeArray(const TArray<unsigned>& SourceArray, int FirstKey, int NumKeys, TArray<float>& Times,
                                   float TimeScale)
{
    ...
    guard(UAnimSequence::DecodeTrans3Anims)
        ...
        switch (RotationCompressionFormat)
        {
            ...
            case ACF_Float32NoW:
                ...
                break;
            default:
                appError("Unknown RotationCompressionFormat %d (%s)", RotationCompressionFormat,
                         EnumToName(RotationCompressionFormat));
        }
        ...
        DBG("          TF3: StatRot=%d StatTrans=%d AnimRot=%d AnimTrans=%d UncompTrans=%d TotalSize=%d (%d)\n",
            NumberOfStaticRotations, NumberOfStaticTranslations, NumberOfAnimatedRotations,
            NumberOfAnimatedTranslations,
            ...
            else
            {
                ...
                for (i = 0; i < NumFrames; i++)
                {
                    ...
                    switch (RotationCompressionFormat)
                    {
                        ...
                        default:
                            appError("Unknown rotation compression method: %d (%s)", RotationCompressionFormat,
                                     EnumToName(RotationCompressionFormat));
                    }
                }
                if (RotationIntervalSize)
...
void UAnimSet::ConvertAnims()
{
	...
	no_track_details: ;
            ...
            {
                appNotify(
                    "AnimSequence %s/%s has wrong CompressedTrackOffsets size (has %d, expected %d), removing track",
            ...
            for (j = 0; j < NumTracks; j++, offsetIndex += offsetsPerBone)
            {
                                ...
                                for (k = 0; k < NumKeys; k++)
                                {
                                    switch (KeyFormat)
                                    {
                                        ...
                                        case ACF_Fixed48NoW:
                                            {
                                                ...
                                                {
                                                    Reader << X;
                                                    v[0] = DecodeFixed48_PerTrackComponent<7>(X);
                                                }
                                                if (ComponentMask & 2)
                                                {
                                                    Reader << Y;
                                                    v[1] = DecodeFixed48_PerTrackComponent<7>(Y);
                                                }
                                                if (ComponentMask & 4)
                                                {
                                                    Reader << Z;
                                                    v[2] = DecodeFixed48_PerTrackComponent<7>(Z);
                                                }
                                                A->KeyPos.Add(v);
                                            }
                                            break;
                                        case ACF_Identity:
                                            ...
                                            break;
                                        default:
                                            appError("Unknown translation compression method: %d (%s)", KeyFormat,
                                                     EnumToName(KeyFormat));
                                    }
                                }
                                // align to 4 bytes
                        ...
                        guard(RotKeys)
                            ...
                            else
                            {
                                ...
                                if (ArGame == GAME_Borderlands || ArGame == GAME_AliensCM) // Borderlands 2
                                {
                                    ...
                                    {
                                        case 6:
                                            KeyFormat = ACF_Delta40NoW;
                                            break; // not used
                                        case 7:
                                            KeyFormat = ACF_Delta48NoW;
                                            break; // not used
                                        case 8:
                                            KeyFormat = ACF_Identity;
                                            break;
                                        case 9:
                                            KeyFormat = ACF_PolarEncoded32;
                                            break;
                                        case 10:
                                            KeyFormat = ACF_PolarEncoded48;
                                            break;
                                    }
                                }
                                #endif // BORDERLANDS
                                ...
                                for (k = 0; k < NumKeys; k++)
                                {
                                    switch (KeyFormat)
                                    {
                                        ...
                                        case ACF_Identity:
                                            ...
                                            break;
                                        default:
                                            appError("Unknown rotation compression method: %d (%s)", KeyFormat,
                                                     EnumToName(KeyFormat));
                                    }
                                }
                                // align to 4 bytes
                ...
                do_not_override_trans_format:
                    ...
                    #if BORDERLANDS
                    FVector Base;
                    if (ArGame == GAME_Borderlands && (TranslationCompressionFormat == ACF_Delta40NoW ||
                        TranslationCompressionFormat == ACF_Delta48NoW))
                    {
                            ...
                            case ATCF_Float16:
                                {
                                    ...
                                    FVector v;
                                    v.X = half2float(x) / 2;
                                    // Argonauts has "half" with biased exponent, so fix it with division by 2
                                    v.Y = half2float(y) / 2;
                                    v.Z = half2float(z) / 2;
                                    A->KeyPos.Add(CVT(v));
                                }
                                ...
                                #endif // ARGONAUTS
                            default:
                                appError("Unknown translation compression method: %d (%s)",
			...
			int TransEnd = Reader.Tell();
                ...
                #if BORDERLANDS
                FQuat Base;
                if (ArGame == GAME_Borderlands && (RotationCompressionFormat == ACF_Delta40NoW ||
                    RotationCompressionFormat == ACF_Delta48NoW))
                {
                ...
                #if TRANSFORMERS
                if (ArGame == GAME_Transformers && RotKeys >= 2 &&
                    (RotationCompressionFormat == ACF_IntervalFixed32NoW || RotationCompressionFormat ==
            ...
            rot_keys_done:
                ...
                {
                    // convert time tracks
                    ReadArgonautsTimeArray(Seq->CompressedTrackTimes, Seq->CompressedTrackTimeOffsets[j * 2], TransKeys,
                                           A->KeyPosTime, Seq->NumFrames);
                    ReadArgonautsTimeArray(Seq->CompressedTrackTimes, Seq->CompressedTrackTimeOffsets[j * 2 + 1],
                                           RotKeys, A->KeyQuatTime, Seq->NumFrames);
                }
                #endif // ARGONAUTS
