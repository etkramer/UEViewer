...
/*static*/
void FMorphTargetDelta::Serialize3(FArchive& Ar, FMorphTargetDelta& V)
{
    Ar << V.PositionDelta;
...
/*static*/
void UMorphTarget::Serialize3(FArchive& Ar)
{
...
struct FSkelMeshSection3
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FSkelMeshSection3& S)
    {
        guard(FSkelMeshSection3<<)
            ...
            #if BATMAN
            if (Ar.Game == GAME_Batman3) goto old_section;
            // Batman1 and 2 has version smaller than 806; Batman3 has outdated code, but Batman4 - new one
            #endif
            #if MKVSDC
...
struct FRigidVertex3
{
    ...
    normals:
        ...
        {
            if (Ar.ArLicenseeVer >= 3 && Ar.ArLicenseeVer <= 52)
                // Frontlines (the code in Homefront uses different version comparison!)
                NumUVSets = 2;
    ...
    friend FArchive& operator<<(FArchive& Ar, FSkelMeshChunk3& V)
    {
        guard(FSkelMeshChunk3<<)
            ...
            DBG_SKEL("Chunk: FirstVert=%d RigidVerts=%d (%d) SoftVerts=%d (%d) MaxInfs=%d\n",
                     V.FirstVertex, V.RigidVerts.Num(), V.NumRigidVerts, V.SoftVerts.Num(), V.NumSoftVerts,
                     V.MaxInfluences);
            return Ar;
        unguard;
    }
    ...
    friend FArchive& operator<<(FArchive& Ar, FSkeletalMeshVertexBuffer3& S)
    {
        ...
        new_version:
            ...
            if (Ar.Game == GAME_LostPlanet3 && Ar.ArLicenseeVer >= 75)
            {
                ...
                Ar << NoUV;
                assert(!NoUV);
                // when NoUV - special vertex format used: FCompNormal[2] + FVector (or something like this?)
            }
            #endif // LOST_PLANET3
            ...
            DBG_SKEL("... data: packUV:%d packPos:%d%s numUV:%d PackPos:(%g %g %g)+(%g %g %g)\n",
                     !S.bUseFullPrecisionUVs, S.bUsePackedPosition, AllowPackedPosition ? "" : " (disabled)",
                     S.NumUVSets,
...
struct FStaticLODModel3
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FStaticLODModel3& Lod)
    {
        guard(FStaticLODModel3<<)
            ...
            //?? remove this code, doesn't work anyway
            {
		...
		appPrintf("Indices: %d (16) / %d (32)\n", Lod.IndexBuffer.Indices16.Num(), Lod.IndexBuffer.Indices32.Num());
            ...
            if (Ar.Game == GAME_EndWar || (Ar.Game == GAME_Borderlands && Ar.ArLicenseeVer >= 57))
            //!! Borderlands version unknown
            {
                // refined field set
            ...
            if (Ar.Game == GAME_Transformers && Ar.ArVer >= 536)
            {
                ...
                assert(Ar.ArLicenseeVer >= 152);
                // has mixed version comparisons - ArVer >= 536 and ArLicenseeVer >= 152
    ...
    guard(USkeletalMesh3::ConvertMesh)
        ...
        for (int lod = 0; lod < LODModels.Num(); lod++)
        {
            guard(ConvertLod)
                ...
                int NumTexCoords = max(SrcLod.NumUVSets, SrcLod.GPUSkin.NumUVSets);
                // number of texture coordinates is serialized differently for some games
    ...
    friend FArchive& operator<<(FArchive& Ar, FStaticMeshSection3& S)
    {
        guard(FStaticMeshSection3<<)
            ...
            if (Ar.Game == GAME_Huxley && Ar.ArVer >= 485)
                return Ar << S.Index; //?? other name?
            #endif // HUXLEY
            if (Ar.ArVer >= 492) Ar << S.Index;
            // real version is unknown! This field is missing in GOW1_PC (490), but present in UT3 (512)
            #if ALPHA_PR
            ...
            // && (Ar.ArLicenseeVer < 102)) - this will drop Batman3 support
            {
                UObject* unk4;
                Ar << unk4;
            }
...
struct FStaticMeshUVItem3
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FStaticMeshUVItem3& V)
    {
        ...
        new_ver:
            ...
            #endif
            #if TRANSFORMERS
            if (Ar.Game == GAME_Transformers && Ar.ArLicenseeVer >= 181) goto uvs;
            // Transformers: Fall of Cybertron, no version in code
            #endif
...
struct FStaticMeshLODModel3
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FStaticMeshLODModel3& Lod)
    {
		...
		for (int i = 0; i < Lod.Sections.Num(); i++)
		{
            ...
            #if BORDERLANDS
            if (Ar.Game == GAME_Borderlands && Ar.ArLicenseeVer >= 57 && Ar.ArVer < 832)
            // Borderlands 1, version unknown; not valid for Borderlands 2
            {
                // refined field set
                Ar << Lod.VertexStream;
                Ar << Lod.UVStream;
