...
template <class T>
struct MotionChunkCompress : public MotionChunkCompressBase
{
    TArray<T> AnimTracks;
    ...
    {
        return Ar << M.RootSpeed3D << M.TrackTime << M.StartBone << M.Flags << M.BoneIndices << M.AnimTracks << M.
            RootTrack;
    }
};
...
// memory.
template <class T>
class TRawArrayUC2 : public TArray<T>
{
...
template <class T>
inline TRawArrayUC2<T>& ToRawArrayUC2(TArray<T>& Arr)
{
...
struct FlexTrackBasePtr
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FlexTrackBasePtr& T)
    {
        guard(SerializeFlexTrack)
            ...
            Ar << TrackType;
            #if UC2
            if (Ar.Engine() == GAME_UE2X && Ar.ArVer >= 130 && TrackType == 4) TrackType = 3;
            // replaced type: FlexTrack48RotOnly -> FlexTrack48
            #endif
