    ...
    guard(FPackageFileSummary<<)
        ...
        if (Tag != PACKAGE_FILE_TAG)
        {
            if (Tag != PACKAGE_FILE_TAG_REV)
            {
                ...
                appNotify("Wrong package tag (%08X) in file %s. Probably the file is encrypted.", Tag,
	...
	appResetProfiler();
        ...
        #endif
        {
            ...
            if (Game >= GAME_UE4_BASE && Summary.IsUnversioned)
                ADD_LOG("[Unversioned] ");
            #endif // UNREAL4
            ADD_LOG("Names: %d Exports: %d Imports: %d Game: %X\n", Summary.NameCount, Summary.ExportCount,
                    Summary.ImportCount, Game);
            // Flush text
            #undef ADD_LOG
            PKG_LOG("%s", LogBuffer);
        }
...
/*static*/
void UnPackage::UnloadPackage(UnPackage* package)
{
    if (package)
    {
        delete package;
...
int UnPackage::FindExportForImport(const char* ObjectName, const char* ClassName, UnPackage* ImporterPackage,
                                   int ImporterIndex)
{
...
void UnPackage::GetFullExportName(const FObjectExport& Exp, char* buf, int bufSize, bool IncludeObjectName,
                                  bool IncludeCookedPackageName) const
{
...
/*static*/
UnPackage* UnPackage::LoadPackage(const CGameFileInfo* File, bool silent)
{
