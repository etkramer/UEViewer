...
void FPackageFileSummary::Serialize2(FArchive& Ar)
{
    guard(FPackageFileSummary::Serialize2)
        ...
        #endif // SPLINTER_CELL
        #if RAGNAROK2
        if ((GForceGame == 0) && (PackageFlags & 0x10000) && (Ar.ArVer >= 0x80 && Ar.ArVer < 0x88))
        //?? unknown upper limit; known lower limit: 0x80
        {
...
void FObjectExport::Serialize2(FArchive& Ar)
{
	...
	if (Ar.Game == GAME_Pariah)
	{
        ...
        if (Ar.Game == GAME_AA2)
        {
            ...
            Ar << ObjectFlags << ObjectName << AR_INDEX(SerialSize);
            // ObjectFlags are serialized in different order, ObjectFlags are negated
            if (SerialSize)
                Ar << AR_INDEX(SerialOffset);
