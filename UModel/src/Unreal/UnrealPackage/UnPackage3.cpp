...
void FPackageFileSummary::Serialize3(FArchive& Ar)
{
    guard(FPackageFileSummary::Serialize3)
        ...
        #if MASSEFF
        if ((((Ar.Game == GAME_MassEffect3 || Ar.Game == GAME_MassEffectLE) && Ar.ArLicenseeVer >= 194)) && (
            PackageFlags & PKG_Cooked)) // ME3 or ME3LE
        {
            int32 unk88;
            Ar << unk88;
        }
    ...
    guard(UnPackage::LoadNameTable3)
        ...
        for (int i = 0; i < Summary.NameCount; i++)
        {
            guard(Name)
                ...
                #endif // R6VEGAS
                #if TRANSFORMERS
                if (Game == GAME_Transformers && ArLicenseeVer >= 181)
                // Transformers: Fall of Cybertron; no real version in code
                {
                    char buf[MAX_FNAME_LEN];
                    int len;
                    *this << len;
