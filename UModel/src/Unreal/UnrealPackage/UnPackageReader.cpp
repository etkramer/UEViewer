        ...
        virtual void Serialize(void* data, int size)
        {
            ...
            for (i = 0, p = (byte*)data; i < size; i++, p++)
            {
                ...
                for (shift = 1, v = b & (b - 1); v; v = v & (v - 1))
                    // shift = number of identity bits in 'v' (but b=0 -> shift=1)
                    shift++;
                b = ROL8(b, shift);
                *p = b;
            }
...
FArchive* UnPackage::CreateLoader(const char* filename, FArchive* baseLoader)
{
    guard(UnPackage::CreateLoader)
        ...
        *Loader << checkDword2;
        ...
        if (checkDword2 == PACKAGE_FILE_TAG || checkDword2 == 0x20000 || checkDword2 == 0x10000)
        // seen 0x10000 in Enslaved/PS3
        {
            //!! NOTES:
    ...
    guard(UnPackage::ReplaceLoader)
        ...
        if ((Game == GAME_Bioshock) && (Summary.PackageFlags & 0x20000))
        {
            ...
            *this << NumChunks;
            ...
            //?? there should be a flag signalling presence of compression structures, because of "Tell()-4"
