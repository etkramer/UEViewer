...
void FPackageFileSummary::Serialize4(FArchive& Ar)
{
    guard(FPackageFileSummary::Serialize4)
        ...
        static const int LatestSupportedLegacyVer = -static_cast<int>(ARRAY_COUNT(legacyVerToEngineVer)) + 1;
        ...
        if (LegacyVersion != -4)
            // UE4 had some changes for version -4, but these changes were reverted in -5 due to some problems
            Ar << VersionUE3;
        ...
        if (Ar.Game == GAME_Gears4 || Ar.Game == GAME_SeaOfThieves) Ar.Seek(Ar.Tell() + 6);
...
void FObjectExport::Serialize4(FArchive& Ar)
{
    guard(FObjectExport::Serialize4)
        ...
        {
            int32 FirstExportDependency, SerializationBeforeSerializationDependencies,
                  CreateBeforeSerializationDependencies;
            int32 SerializationBeforeCreateDependencies, CreateBeforeCreateDependencies;
            Ar << FirstExportDependency << SerializationBeforeSerializationDependencies <<
                CreateBeforeSerializationDependencies;
...
void UnPackage::LoadPackageIoStore()
{
	...
	appPrintf("Loading AsyncPackage %s\n", *GetFilename());
        ...
        LoadExportTableIoStore(HeaderData + Sum.ExportMapOffset, ExportCount, ExportTableSize, HeaderSize,
                               BundleHeadersArray, BundleEntriesArray);
...
void UnPackage::LoadExportTableIoStore(const byte* Data, int ExportCount, int TableSize, int PackageHeaderSize,
                                       const TArray<FExportBundleHeader>& BundleHeaders,
                                       const TArray<FExportBundleEntry>& BundleEntries)
{
...
struct ImportHelper
{
    ...
    ImportHelper(const TArray<const CGameFileInfo*>& InPackageFiles, FObjectImport* InImportTable,
                 const FPackageObjectIndex* InImportMap, int InImportCount)
    ...
    bool FindObjectInPackages(FPackageObjectIndex ObjectIndex, int& OutPackageIndex,
                              const FObjectExport*& OutExportEntry) const
    {
...
void UnPackage::LoadImportTableIoStore(const byte* Data, int ImportCount,
                                       const TArray<const CGameFileInfo*>& ImportPackages,
                                       CImportTableErrorStats& ErrorStats)
{
...
/*static*/
void UnPackage::LoadGlobalData4(FArchive* NameAr, FArchive* MetaAr, int NameCount)
{
