...
void FByteBulkData::SerializeHeader(FArchive& Ar)
{
	...
	DUMP_ARC_BYTES(Ar, 32, "Bulk");
        ...
        else
        {
            ...
            BulkDataOffsetInFile = tmpBulkDataOffsetInFile32;
            // sign extend to allow non-standard TFC systems which uses '-1' in this field
        ...
        #endif // UNREAL4
        {
            ...
            serialize_separate_data:
                ...
                if (BulkDataOffsetInFile + BulkDataSizeOnDisk != Ar.Tell64())
                {
                    ...
                    appNotify("Serialize bulk data: current position %llX, expected %llX", Ar.Tell64(),
                              BulkDataOffsetInFile + BulkDataSizeOnDisk);
                }
            }
            else
            {
