...
static bool UploadTex(UUnrealMaterial* Tex, GLenum target, CTextureData& TexData, bool doMipmap, int slice = -1)
{
    guard(UploadTex)
        ...
        /*------------------ Upload the image ---------------------------*/
        // First mipmap
        DBG("up_uncomp %X (%s, %d mips): %d %d (%d)", target, Tex->Name, TexData.Mips.Num(), Mip0.USize, Mip0.VSize,
            TexData.Mips.Num());
        glTexImage2D(target, 0, format, scaledWidth, scaledHeight, 0, GL_RGBA,
                     floatTexture ? GL_FLOAT : GL_UNSIGNED_BYTE, pic);
        ...
        else if (doMipmap)
        {
            guard(BuildMips)
                ...
                while (scaledWidth > 1 || scaledHeight > 1)
                {
                    ...
                    glTexImage2D(target, mipLevel, format, scaledWidth, scaledHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                 pic);
                }
            unguard;
        }
...
static bool UploadCompressedTex(UUnrealMaterial* Tex, GLenum target, GLenum target2, CTextureData& TexData,
                                bool doMipmap, int slice = -1)
{
	...
	return false; // always decompress
        ...
        {
            // no mipmaps required
            DBG("up (%s, %d no-mips): %d %d (%d) (%s) (0x%X)", Tex->Name, TexData.Mips.Num(), Mip0.USize, Mip0.VSize,
                TexData.Mips.Num(), TexData.OriginalFormatName, Mip0.DataSize);
        ...
        else if (TexData.Mips.Num() > 1 && GL_SUPPORT(QGL_1_2)) // GL 1.2 is required for GL_TEXTURE_MAX_LEVEL
        {
            guard(UploadMips)
                ;
                // has mipmaps
                DBG("up (%s, %d mips): %d %d (%d) (%s) (0x%X)", Tex->Name, TexData.Mips.Num(), Mip0.USize, Mip0.VSize,
                    TexData.Mips.Num(), TexData.OriginalFormatName, Mip0.DataSize);
        ...
        else if (GL_SUPPORT(QGL_EXT_FRAMEBUFFER_OBJECT))
        {
            ...
            DBG("up+build_mips (%s): %d %d (%d) (%s) (%d)", Tex->Name, Mip0.USize, Mip0.VSize, TexData.Mips.Num(),
                TexData.OriginalFormatName, Mip0.DataSize);
            glCompressedTexImage2D(target, 0, format, Mip0.USize, Mip0.VSize, 0, dataSize, dataPtr);
...
static int Upload2D(UUnrealMaterial* Tex, bool doMipmap, bool clampS, bool clampT)
{
    guard(Upload2D)
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, doMipmap ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
        // trilinear filter
		...
		*pic2++ = 255;
        ...
        if (side == 5)
        {
            ...
            glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MIN_FILTER,
                            doMipmap ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR); // trilinear filter
...
void GL_NormalmapShader(CShader& shader, CMaterialParams& Params)
{
    guard(GL_NormalmapShader)
        ...
        if (Params.Normal)
        //!! reimplement ! plus, check for correct normalmap texture (VTC texture compression etc ...)
        {
        ...
        if (Params.Specular)
        {
            ...
            if (Params.Specular->Bind())
                specularExpr = va("texture2D(specTex, TexCoord).%s * vec3(specular) * 1.5",
                                  !Params.SpecularFromAlpha ? "rgb" : "a");
        }
        else if (Params.SpecPower)
        {
            ...
            if (bHasDiffuse && Params.PBRMaterial && Params.SpecPower->Bind())
            {
                ...
                specularExpr =
                    "(GetMaterialDiffuseColor(TexCoord).rgb * texture2D(spPowTex, TexCoord).r * 0.2) * gl_FrontMaterial.shininess";
            }
            else
            {
        ...
        if (Params.SpecPower)
        {
            ...
            if (Params.SpecPower->Bind())
            {
                if (!Params.PBRMaterial)
                    ...
                    specPowerExpr =
                        "(2.0 / pow(texture2D(spPowTex, TexCoord).r, 4.0) - 2.0) * gl_FrontMaterial.shininess / 10.0";
            }
        }
        ...
        {
            static const char* mobileSpecExpr[] = {
                // EMobileSpecularMask values
...
void UMaterial3::GetParams(CMaterialParams& Params) const
{
    guard(UMaterial3::GetParams)
        ...
        int DiffWeight = 0, NormWeight = 0, SpecWeight = 0, SpecPowWeight = 0, OpWeight = 0, EmWeight = 0, CubeWeight =
                0;
...
void UMaterialInstanceConstant::GetParams(CMaterialParams& Params) const
{
    guard(UMaterialInstanceConstant::GetParams)
        ...
        int DiffWeight = 0, NormWeight = 0, SpecWeight = 0, SpecPowWeight = 0, OpWeight = 0, EmWeight = 0, EmcWeight = 0
          , CubeWeight = 0, MaskWeight = 0;
