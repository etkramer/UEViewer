        ...
        for (int lod = 0; lod < Lods.Num(); lod++)
        {
            ...
            for (int vert = 0; vert < L.NumVerts; vert++, V++)
            {
                ...
                *(uint32*)UnpackedWeights = V->PackedWeights;
                ...
                if (ShouldFix)
                {
                    for (int i = NUM_INFLUENCES - 1; i >= 0; i--)
                    // iterate in reverse order for correct removal of '0' followed by '0'
                    {
                        if (UnpackedWeights[i] == 0)
                        {
...
static void GetKeyParams(const TArray<float>& KeyTime, float Frame, float NumFrames, bool Loop, int& X, int& Y,
                         float& F)
{
    guard(GetKeyParams)
        ;
