...
void UObject::GetFullName(char* buf, int bufSize, bool IncludeObjectName, bool IncludeCookedPackageName,
                          bool ForcePackageName) const
{
...
void UObject::EndLoad()
{
	...
	#define EDITOR_STR		""
                ...
                unguardf("%s'%s.%s', pos=%X, ver=%d/%d%s%s, game=%s", Obj->GetClassName(), Package->Name, Obj->Name,
                             Package->Tell(),
            ...
            if (Ar.Game >= GAME_UE3)
            {
                ...
                if (Ar.Game == GAME_MK && Ar.ArVer >= 677)
                {
                    ...
                    else if (PropType == "BoolProperty")
                    {
                        ...
                        Ar << Tag.BoolValue;
                        // as original code, but 'int' value (would be serialized as byte otherwise)
                        return Ar;
                    }
                }
                #endif // MKVSDC
...
struct FPropertyTagBat2
{
    ...
    friend FArchive& operator<<(FArchive& Ar, FPropertyTagBat2& Tag)
    {
        ...
        if (Ar.Game == GAME_Batman3 && Ar.ArLicenseeVer >= 104) goto named_prop;
        // Batman3 Online has old version of FPropertyTag
        Ar << Tag.Offset;
        if (Tag.Type == NAME_IntProperty || Tag.Type == NAME_FloatProperty || Tag.Type == NAME_NameProperty || Tag.Type
            == NAME_VectorProperty ||
            Tag.Type == NAME_RotatorProperty || Tag.Type == NAME_StrProperty || Tag.Type == 16
            /*NAME_ObjectNCRProperty*/)
        {
        simple_prop:
        ...
        // serialized as 4 byte int, i.e. a bunch of boolean properties
...
void CTypeInfo::ReadUnrealProperty(FArchive& Ar, FPropertyTag& Tag, void* ObjectData, int PropTagPos) const
{
	...
	appPrintf("-- %s \"%s::%s\" [%d] size=%d enum=%s struc=%s\n", DbgTypeName, Name, *Tag.Name,
        ...
        if (Tag.Type == NAME_ArrayProperty)
        {
            ...
            {
                appPrintf(
                    "ERROR: Struct \"%s\": %s %s has Prop.Count=%d (should be -1) and ArrayIndex=%d (should be 0). Skipping property.\n",
	...
	if (!COMPARE_TYPE(Prop->TypeName, TypeConst)) \
		...
		if (COMPARE_TYPE(Prop->TypeName, TypeConst)) { Ar << *(TArray<CppType>*)Arr; }
                    ...
                    SIMPLE_ARRAY_TYPE(PropType::Int, int)
                        ...
                        SIMPLE_ARRAY_TYPE(PropType::Bool, bool)
                        else
                            SIMPLE_ARRAY_TYPE(PropType::Byte, byte)
                            else
                                SIMPLE_ARRAY_TYPE(PropType::Float, float)
                                else
                                    SIMPLE_ARRAY_TYPE(PropType::UObject, UObject*)
                                    else
                                        SIMPLE_ARRAY_TYPE(PropType::FName, FName)
                                        else
                                            SIMPLE_ARRAY_TYPE(PropType::FVector, FVector)
                                            else
                                                SIMPLE_ARRAY_TYPE("FQuat", FQuat)
                                                else
                                                {
                                                    ...
                                                    #if UNREAL4
                                                    if (Ar.Engine() >= GAME_UE4_BASE && Ar.ArVer >=
                                                    ...
                                                    {
                                                        appPrintf(
                                                            "WARNING: structure type %s is unknown, skipping array %s::%s\n",
                                                    ...
                                                    else if (DataCount)
                                                    {
                                                        ...
                                                        #endif // UNREAL4
                                                        {
                                                            ...
                                                            for (int i = 0; i < DataCount; i++, item += ItemType->
                                                                 SizeOf)
                                                            {
                                                                #if DEBUG_PROPS
							appPrintf("Item[%d]:\n", i);
                                                                ...
                                                                if (ItemType->Constructor)
                                                                    ItemType->Constructor(item);
                                                                    // fill default properties
                                                                else
                                                                    memset(item, 0, ItemType->SizeOf);
                                                                // no constructor, just initialize with zeros
            ...
            case NAME_StructProperty:
                {
                    ...
                    if (stricmp(Prop->TypeName + 1, *Tag.StrucName) != 0 && stricmp(*Tag.StrucName, "None") != 0)
                    // Tag.StrucName could be unknown in Batman2
                    {
                        appNotify("Struc property %s expected type %s but read %s", *Tag.Name, Prop->TypeName,
                                  *Tag.StrucName);
                        Ar.Seek(StopPos);
                    }
		...
		DUMP_ARC_BYTES(Ar, StopPos - PropTagPos, "Property bytes");
            #endif
            appError("%s\'%s\'.%s: Property read error: %d unread bytes", Name, UObject::GLoadingObj->Name, *Tag.Name,
                     StopPos - Pos);
        }
		...
		appPrintf("Prop: type=%d (%s) offset=0x%X size=%d propName=%s\n", TagBat.Type, TypeName, TagBat.Offset, TagBat.DataSize, *TagBat.PropertyName);
            ...
            switch (TagBat.Type)
            {
                ...
                case NAME_RotatorProperty:
                    Ar << PROP(FRotator);
                // UE4 has the same memory layout, just int32 is replaced with float, so don't bother about different serializer
