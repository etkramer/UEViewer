...
FORCEINLINE uint32 GetHashInternal(const char* s, int len)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        hash = ROL16(hash, 1) + c;
        // note: if we'll use more than 16-bit GAME_FILE_HASH_SIZE value, should use ROL32 here
    }
    return hash;
}
    ...
    guard(CGameFileInfo::Register)
        ...
        if (!parentVfs && !IsPackage)
        {
            ...
            #endif
            {
                ...
                if (appStristr(RegisterInfo.Filename, "cooked") || appStristr(RegisterInfo.Filename, "content")) return
                    NULL;
...
void appSetRootDirectory(const char* dir, bool recurse)
{
    guard(appSetRootDirectory)
        ...
        appPrintf("Found %d game files (%d skipped) in %d folders at path \"%s\"\n", GameFiles.Num(), GNumForeignFiles,
                  GameFolders.Num() ? GameFolders.Num() - 1 : 0, dir);
...
/*static*/
const FString& CGameFileInfo::GetPathByIndex(int index)
{
...
/*static*/
int CGameFileInfo::CompareNames(const CGameFileInfo& A, const CGameFileInfo& B)
{
