...
struct FIoOffsetAndLength
{
    uint64 GetOffset() const
    {
        ...
        return (uint64(Data[0]) << 32) | (uint64(Data[1]) << 24) | (uint64(Data[2]) << 16) | (uint64(Data[3]) << 8) |
            uint64(Data[4]);
    }
    ...
    {
        // 5 byte big-endian value
        return (uint64(Data[5]) << 32) | (uint64(Data[6]) << 24) | (uint64(Data[7]) << 16) | (uint64(Data[8]) << 8) |
            uint64(Data[9]);
    }
    ...
    bool Read(FArchive& Ar, FString& PakEncryptionKey)
    {
        guard(FIoStoreTocResource::Read)
            ...
            if (Header.ContainerFlags & (int)EIoContainerFlags::Indexed)
            {
                ...
                {
                    appDecryptAES(const_cast<byte*>(DataPtr), Header.DirectoryIndexSize, &PakEncryptionKey[0],
                                  PakEncryptionKey.Len());
                }
...
void FIOStoreFile::Serialize(void* data, int size)
{
    ...
    guard(FIOStoreFile::Serialize)
        ...
        while (size > 0)
        {
            if ((UncompressedBuffer == NULL) || (ArPos < UncompressedBufferPos) || (ArPos >= UncompressedBufferPos +
                Parent->CompressionBlockSize))
            {
                // buffer is not ready
                if (UncompressedBuffer == NULL)
                {
                    UncompressedBuffer = (byte*)appMallocNoInit(Parent->CompressionBlockSize);
                ...
                {
                    // Compressed data
                    assert(CompressionMethodIndex <= Parent->NumCompressionMethods);
                    // 0 = None is not counted, so "<=" is used here
                    int CompressionFlags = Parent->CompressionMethods[CompressionMethodIndex];
                    appDecompress(CompressedData, CompressedBlockSize, UncompressedBuffer, UncompressedBlockSize,
                                  CompressionFlags);
            ...
            int BytesToCopy = UncompressedBufferPos + Parent->CompressionBlockSize - ArPos;
            // number of bytes until end of the buffer
...
void FIOStoreFileSystem::WalkDirectoryTreeRecursive(struct FIoDirectoryIndexResource& IndexResource, int DirectoryIndex,
...
/*static*/
bool FIOStoreFileSystem::LoadGlobalContainer(const char* Filename)
{
