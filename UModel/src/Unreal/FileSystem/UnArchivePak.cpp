    ...
    guard(FPakFile::Serialize)
        ...
        if (Info->CompressionMethod)
        {
            guard(SerializeCompressed)
                ...
                while (size > 0)
                {
                    if ((UncompressedBuffer == NULL) || (ArPos < UncompressedBufferPos) || (ArPos >=
                        UncompressedBufferPos + Info->CompressionBlockSize))
                    {
                        // buffer is not ready
                        if (UncompressedBuffer == NULL)
                        {
                            UncompressedBuffer = (byte*)appMallocNoInit((int)Info->CompressionBlockSize);
                        ...
                        int UncompressedBlockSize = min((int)Info->CompressionBlockSize,
                                                        (int)Info->UncompressedSize - UncompressedBufferPos);
                        // don't pass file end
                        byte* CompressedData;
                        if (!Info->bEncrypted)
                        {
                    ...
                    int BytesToCopy = UncompressedBufferPos + Info->CompressionBlockSize - ArPos;
            ...
            guard(SerializeEncrypted)
                ...
                while (size > 0)
                {
                    if ((ArPos < UncompressedBufferPos) || (ArPos >= UncompressedBufferPos + EncryptedBufferSize))
                    {
                        ...
                        RemainingSize = Align(RemainingSize, EncryptionAlign);
                        // align for AES, pak contains aligned data
                    ...
                    int BytesToCopy = UncompressedBufferPos + EncryptedBufferSize - ArPos;
                    // number of bytes until end of the buffer
                    if (BytesToCopy > size) BytesToCopy = size;
                    assert(BytesToCopy > 0);
...
bool FPakVFS::DecryptPakIndex(TArray<byte>& IndexData, FString& ErrorString)
{
    guard(FPakVFS::DecryptPakIndex)
        ...
        {
            char buf[1024];
            appSprintf(ARRAY_ARG(buf), "WARNING: The provided encryption key doesn't work with \"%s\". Skipping.",
                       *Filename);
            ErrorString = buf;
            return false;
        }
        ...
        for (int i = 0; i < count; i++)
        {
            guard(ReadInfo)
                ...
                if (info.Version >= PakFile_Version_FNameBasedCompressionMethod)
                {
                    ...
                    E.CompressionMethod = CompressionMethodIndex > 0
                                              ? info.CompressionMethods[CompressionMethodIndex - 1]
                                              : 0;
                }
