            ...
            case AZRC_QuatRelative_32:
                {
                    ...
                    for (int i = 0; i < T.NumKeys; i++)
                    {
                        ...
                        CQuat q = FinishQuatRelative(10, (val32 >> 20) & 0x3FF, (val32 >> 10) & 0x3FF, val32 & 0x3FF,
                                                     Interval);
                        Track.KeyQuat.Add(q);
                    }
                }
            ...
            case AZRC_QuatRelative_16:
                {
                    ...
                    for (int i = 0; i < T.NumKeys; i++)
                    {
                        ...
                        CQuat q = FinishQuatRelative(5, (val32 >> 10) & 0x1F, (val32 >> 5) & 0x1F, val32 & 0x1F,
                                                     Interval);
                        Track.KeyQuat.Add(q);
                    }
                }
...
void UAnimSequence::DecodeBatman2Anims(CAnimSequence* Dst, UAnimSet* Owner) const
{
	...
	memset(&TranslationCodecUsed, 0, sizeof(TranslationCodecUsed));
        ...
        for (Bone = 0; Bone < NumTracks; Bone++)
        {
            ...
            int RotationCodec = FindAndDecodeRotation(Bone, Reader, *Track, Hdr.RotationOffset, Hdr.RotationCount,
                                                      Owner);
            int TranslationCodec = FindAndDecodeTranslation(Bone, Reader, *Track, Hdr.TranslationOffset,
                                                            Hdr.TranslationCount, Owner);
            // fix quaternions
            if (Bone > 0)
            {
