...
//			*(byte**)(SectionStart + VF->fromOffset) = SectionStart2 + VF->toOffset;
            ...
            #endif // DUMP_TYPEINFO
            if (Sec->m_exportsOffset != Sec->m_importsOffset) appNotify("%s, Sec[%s] has exports", Name,
                                                                        Sec->m_sectionTag);
...
void GetHavokPackfileContents(const void* PackData, void** Object, const char** ClassName)
{
    guard(GetHavokPackfileContents)
        ...
        hkPackfileHeader* Hdr = (hkPackfileHeader*)PackStart;
        hkPackfileSectionHeader* Sections = (hkPackfileSectionHeader*)(Hdr + 1);
        *Object = (char*)PackStart + Sections[Hdr->m_contentsSectionIndex].m_absoluteDataStart + Hdr->
            m_contentsSectionOffset;
        *ClassName = (char*)PackStart + Sections[Hdr->m_contentsClassNameSectionIndex].m_absoluteDataStart + Hdr->
